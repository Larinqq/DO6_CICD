...
compare_output() {
    ...
    ../../build/s21_cat -"$flags" "$file" > s21_cat_output.txt 2>/dev/null
...
run_tests() {
    ...
    for file in "${test_files[@]}"; do
        cp "$CI_PROJECT_DIR/tests/$file" . 2>/dev/null || true
    done
    
    echo -e "\n CAT TESTING SINGLE AND COMBINED FLAGS\n"
    ...
    echo -e "\n CAT TESTING LONG FLAGS (GNU-style)\n"

    for file in "${test_files[@]}"; do
        for flag in "${long_flags[@]}"; do
            ((test_num++))
            cat "$flag" "$file" > cat_output.txt 2>/dev/null
            ../../build/s21_cat "$flag" "$file" > s21_cat_output.txt 2>/dev/null
